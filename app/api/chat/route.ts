...
요구사항: ${cleanPrompt}`;
    ...
    // 🌟 키의 생김새(startsWith)가 아닌, 프론트에서 요구한 모델(requestedModel)을 최우선 기준으로 분기합니다.
    if (requestedModel === 'claude' || apiKey.startsWith('sk-ant')) {
      // 🌟 [에이전트 협업 파이프라인 시동]
    ...
    else if (requestedModel === 'xai' || apiKey.startsWith('xai-') || apiKey.includes('x.ai')) {
