...
export async function POST(req: Request) {
  try {
    // 🌟 [수정] 프론트에서 보낸 requestedModel을 여기서 한 번에 같이 꺼내옵니다.
    const { prompt: rawPrompt, encryptedApiKey, history = [], images = [], requestedModel } = await req.json();
...
요구사항: ${cleanPrompt}`;
    ...
    if (requestedModel === 'claude') {
      ...
      if (rawPrompt.startsWith('/그림 ') || (!isUIRequest && (rawPrompt.includes('그려') || rawPrompt.includes('이미지') || rawPrompt.includes('사진') || rawPrompt.includes('배경화면')))) {
        ...
        // 🌟 [핵심] 이전 대화 기록을 싹 다 긁어모아 번역기에게 '문맥'을 주입합니다. (연속 수정 가능)
        ...
        translationHistory.push({
          role: 'user',
          content: `이전 대화 문맥(Context)을 완벽히 파악하여, 다음 그림/수정 요청을 AI 이미지 생성기용 고품질 영어 프롬프트로 번역 및 확장해줘. (수정 요청일 경우 기존 형태를 유지하면서 변경점만 적용할 것). 다른 말은 절대 하지 말고 오직 영어 프롬프트만 출력해: ${userImagePrompt}`
        });
